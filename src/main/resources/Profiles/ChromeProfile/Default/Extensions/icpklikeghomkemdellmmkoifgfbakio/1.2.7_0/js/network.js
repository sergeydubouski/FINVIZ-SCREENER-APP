...
anonymox.Network = (function () {
	...
	function getInfoError (error, username, passwordPlain, callbackFn) {
		...
		// otherwise retry
			...
			// re-request userdata on CE01
			if (error instanceof GenericException2 && error.xmessage && error.xmessage.startsWith("CE01")) {
				console.log("anonymox.Network.getInfo re-requesting userdata");
				anonymox.Network.getAccount(function (username, passwordPlain) {
					var opt = anonymox.Model.Options;
					opt.username = username;
					opt.passwordPlain = passwordPlain;
					opt.save();
